...
function showNestedCategories(){
	...
	$db = Database::getInstance();
	$mysqli = $db->getConnection();
	$query = "select category.*, catetory_relations.ParentcategoryId
				from category
	...
	function fetchCategoryTreeList($parent = 0, $user_tree_array = '') {
		...
		$query = "SELECT COUNT(DISTINCT ItemNumber)
				FROM (
				  (SELECT ItemNumber
				  ...
				  FROM Item_category_relations pc
		...
		while($res=$stmt->fetch_object()){
			$list .="<li>".$res->Name."(100)</li>";
		}
        $list .="</ul>";
		//$user_tree_array = array();

		/*while ($row = $stmt->fetch_object()) {
			$user_tree_array[$row->Name]['item'] = 100;
			//$sub_categories = $this->fetchChildCategoryList($row->id);
			//$user_tree_array[$row->Name]['sub_categories'] = $sub_categories;
		}*/
	...
	function fetchChildCategoryList($parent = 0) {
		...
		while ($row = $stmt->fetch_object()) {
			$child_user_tree_array[$row->Name]['item'] = 100;
		}
	}
